...
- **`out/`**: Default output directory for Dell artifacts produced by the adapter and diagnostics scripts (canonical/items/summary, segmentation, materialized segments, cleaned spec). This is the primary consumption/inspection location for the current pipeline scripts.
- **`diag/`**: Diagnostics/batch-run folders. Only used when the Dell batch runner copies artifacts for a specific input; otherwise it is not written to. In the referenced run bundle, only `out/` is present; `diag/` is unused.
...
| `cleaned_spec.dell.segment_<segment_id>.xlsx` | `diag/<input_basename>/` | Dell batch runner: `node scripts/diagnostics/dell-batch.js` (copies `.xlsx` from `out/` into per-input diag folder) | Diagnostic copy of the cleaned spec for each batch input. | OPTIONAL/DIAGNOSTIC | Copy of the `out/` artifact for batch inspection. |
